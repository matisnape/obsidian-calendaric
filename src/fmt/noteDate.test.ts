...
describe("computeNoteDate", () => {
	it("AC-FMT-06.1: collapses two times on the same calendar day to the same day-note date", () => {
	...
	it("AC-FMT-06.2: does not mutate the input date", () => {
	...
	it("AC-FMT-06.3: produces different note dates for different granularities of the same instant", () => {
		const date = moment("2026-04-13T14:30:00");

		expect(computeNoteDate(date, "day")).not.toBe(computeNoteDate(date, "week"));
	});

	it("AC-FMT-06.4: defaults to day granularity when none is given", () => {
		const date = moment("2026-04-13T14:30:00");
	...
	it("AC-FMT-06.5: keeps two instants a few milliseconds apart on opposite sides of midnight on different note dates", () => {
