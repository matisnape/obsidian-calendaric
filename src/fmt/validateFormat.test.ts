...
/** Render the tab, type `value` into the day format field, and commit it. */
async function enterDayFormat(value: string) {
	const app = makeApp();
	const settings = structuredClone(DEFAULT_SETTINGS);
	settings.day.format = "YYYY-MM-DD";
	let saves = 0;
	const plugin = {
		app,
		settings,
	...
	const problems = Array.from(tab.containerEl.querySelectorAll(".calendaric-setting-problem"))
		.map((el) => el.textContent ?? "")
		.filter((text) => text !== "");
	return { saved: settings.day.format, saves, problems };
}
...
describe("the format field on the settings screen", () => {
	it("AC-FMT-05.4: an emptied format keeps the previous value saved and says why", async () => {
		const { saved, saves, problems } = await enterDayFormat("");
	...
	it("AC-FMT-05.1: an illegal character keeps the previous value saved and says why", async () => {
		...
		expect(saves).toBe(0);
		expect(problems).toHaveLength(1);
		expect(problems[0]).toContain("cannot appear in a filename");
	});
	...
	it("AC-FMT-05.2: a format that cannot identify a note is saved, with the warning shown", async () => {
		...
		expect(saves).toBe(1);
		expect(problems).toHaveLength(1);
		expect(problems[0]).toContain("cannot uniquely identify a note");
	});
