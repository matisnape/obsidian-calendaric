...
function dayPath(dayOfMonth: number): string {
	return computeNotePath(firstOfMonth().add(dayOfMonth - 1, "day"), "day", SETTINGS.day, new FakeVaultConfigPort());
}
...
function secondRowWeekPath(): string {
	...
	return computeNotePath(getWeekAnchor(grid[1]!.days), "week", SETTINGS.week, new FakeVaultConfigPort());
}
