# Capability map: the calendar and periodic-notes family

## TL;DR

`calendaric-map.json` describes the useful behaviour of four plugins and
libraries, what Calendaric already implements, and how the live vault uses
them. It gives an agent the evidence needed to reimplement each behaviour
without rereading the original repository.
...
| **Periodic note** | A note that stands for one period, for example one day's note. |
...
| `calendaric` | This repository, as it stands today. The implementation target. |
...
- `capabilities` — the reimplementation checklist. One discrete behaviour per record, with `path:line`.
...
- `observations` — rewrite risks, ranked `P1` / `P2` / `P3`, each with a permanent `OBS-<source>-<nn>` id.
...
Start with `observations` filtered to `P1`. Those are the risks most likely to
break a rewrite that ignores the source evidence.

## How it was built

Seven agents read the repositories in parallel, each writing one file under
`sources/`. `merge.py` combines them, folds the per-agent ids for external
